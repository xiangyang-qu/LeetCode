...
// 
//
// Related Topics 贪心 数组 双指针 排序 👍 585 👎 0
...
class Solution {
    public int findContentChildren(int[] g, int[] s) {
		int[] geedFactors = Arrays.stream(g).sorted().toArray();
		int[] cookies = Arrays.stream(s).sorted().toArray();
		int index = cookies.length - 1;
		int result = 0;
		for (int i = geedFactors.length - 1; i >= 0; i--) {
			//找到刚好满足胃口值的饼干，贪心贪在为了不浪费饼干尺寸。要求饼干正好不小于胃口值
			if (index >= 0 && cookies[index] >= geedFactors[i]) {
				index--;
				result++;
			}
		}
